...
class Castle extends Phaser.GameObjects.Container {
    ...
    setLevel(level) {
        ...
        // Range: base 150, scales to 742 (90% of track) at max level 60
        const baseRange = 150;
        const maxRange = 742;  // 90% of track (824px)
        const maxLevel = 60;   // Max castle level with upgrades
        const rangeIncrements = Math.floor(this.level / 10);  // Every 10 levels (0-6 increments)
        const bonusPerIncrement = (maxRange - baseRange) / 6;  // ~99px per 10 levels
        this.attackRange = Math.min(baseRange + rangeIncrements * bonusPerIncrement, maxRange);
